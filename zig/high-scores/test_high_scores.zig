...
test "Latest score when there are no scores" {
    const scores = &[_]i32{};
    try testing.expectEqual(null, HighScores.init(scores).latest());
}
...
test "Personal best when there are no scores" {
    const scores = &[_]i32{};
    try testing.expectEqual(null, HighScores.init(scores).personalBest());
}

test "Top 3 scores-Personal top when there are no scores" {
    const scores = &[_]i32{};
    try testing.expectEqualSlices(i32, &[_]i32{}, HighScores.init(scores).personalTopThree());
}
